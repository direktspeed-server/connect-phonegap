...
function addDesktopBrowser(options) {
    console.log('*** testing new browser add');
    // Define options
    var opts = [];
    ...
    var command = path.join("/Users/spindori/Documents/PhoneGap/phonegap-app-desktop/build/PhoneGap-darwin-x64/PhoneGap.app/Contents/Frameworks/PhoneGap\ Helper.app/Contents/MacOS/PhoneGap Helper");
    // command += ' /Users/spindori/Documents/PhoneGap/phonegap-cli/bin/phonegap.js';
    // command += ' platform add browser';

    var commandArgs = [path.join('/Users/spindori/Documents/PhoneGap/phonegap-cli/bin/phonegap.js')];
    commandArgs.push('platform');
    commandArgs.push('add');
    commandArgs.push('browser');

    opts.env.PATH = process.env.PATH += ':"' + path.resolve(command, '..') + '"';

    console.log('*** command: ', command);

    var spawn = require('child_process').spawn;
    var child = spawn(command, commandArgs, opts);
    ...
    child.on('error', function(e) {
      console.log('browser child process error:', e);
    });
